...
//        let build = dictionary["Test"] as? String
    ...
    func preloadDBData() {
        let sqlitePath = Bundle.main.path(forResource: "MyDB", ofType: "store")
        ...
        let URL4 = URL(fileURLWithPath: NSPersistentContainer.defaultDirectoryURL().relativePath + "/MyDB.store")
        ...
        if FileManager.default.fileExists(atPath: NSPersistentContainer.defaultDirectoryURL().relativePath + "/MyDB.store") {
            do {
...
//      let documentTypes = Bundle.main.infoDictionary!["CFBundleDocumentTypes"] as! [[String: AnyObject]]
      let extensions = ["store"] //documentTypes.first!["CFBundleTypeExtensions"] as! [String]
